...
export const Services = () => {
  ...
  const changeService = async (serviceId) => {
    try {
      ...
      const updated = await updateService(sessionStorage.getItem("token"), serv, serviceId);
      setDisabled("");
    } catch (error) {
      console.log(error);
    }
              ...
              <article key={index} className="serviceCard">
                <div className="title">
                  <CustomAreaText
                    ...
                    disabled={parseInt(disabled) === service.id ? "" : "disabled"}
                ...
                {sessionStorage.getItem("role") === "super_admin" && (
                  <div className="adminButtons">
                    <Button
                      text={parseInt(disabled) === service.id ? "Guardar" : "Editar"}
                      functionClick={
                        parseInt(disabled) === service.id
                          ? () => changeService(service.id)
                          : () => updateDisabled(service.id, service.serviceName, service.description)
                      }
                      currentClass={
                        parseInt(disabled) === service.id
